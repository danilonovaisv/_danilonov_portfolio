...
const HeroCopy: React.FC<{ className?: string }> = ({ className }) => {
  ...
  useMotionValueEvent(scrollYProgress, 'change', (latest) => {
    if (videoRef.current) {
      // Unmute only when scrolled into the section AND not yet leaving it (0.9 threshold)
  ...
  return (
    <div className={`w-full ${className}`}>
      ...
      <motion.div
        ...
        className={`container mx-auto px-6 md:px-12 lg:px-16 h-full z-10 pointer-events-none ${isVisible ? 'hero-text-visible' : ''}`}
      >
        ...
        <div className="flex flex-col justify-center items-start h-full pt-20 max-w-4xl pointer-events-auto">
          {/* Título Principal */}
          <div className="text-[clamp(3.5rem,10vw,7.5rem)] font-extrabold tracking-[-0.04em] mb-6 md:mb-10 font-sans flex flex-col items-start gap-1">
      ...
      {/* Video layer that appears during scroll */}
      <motion.div
        style={{
          scale: videoScale,
          x: videoX,
        ...
        className="fixed z-40 w-full h-full flex items-center justify-center overflow-hidden shadow-2xl origin-center bg-black/5 pointer-events-none"
      >
