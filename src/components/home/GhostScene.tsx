import React, { useRef, useMemo, useState } from 'react';
import { useFrame } from '@react-three/fiber';
import { Sparkles } from '@react-three/drei';
import * as THREE from 'three';
import {
  EffectComposer,
  Bloom,
  Noise,
  Vignette,
  Scanline,
} from '@react-three/postprocessing';
import { useIsMobile } from '../../hooks/useIsMobile';

const GhostMesh = () => {
  const meshRef = useRef<THREE.Mesh>(null);

  // Custom Geometry with Wavy Bottom
  const geometry = useMemo(() => {
    const geo = new THREE.SphereGeometry(2, 64, 64);
    const positionAttribute = geo.getAttribute('position');
    const positions = positionAttribute.array;

    // Modify vertices for wavy bottom
    for (let i = 0; i < positions.length; i += 3) {
      // positions[i+1] is y
      if (positions[i + 1] < -0.2) {
        const x = positions[i];
        const z = positions[i + 2];
        const noise1 = Math.sin(x * 5) * 0.35;
        const noise2 = Math.cos(z * 4) * 0.25;
        const noise3 = Math.sin((x + z) * 3) * 0.15;
        const combinedNoise = noise1 + noise2 + noise3;
        positions[i + 1] = -2.0 + combinedNoise;
      }
    }

    geo.computeVertexNormals();
    return geo;
  }, []);

  // Animate floating
  useFrame((state) => {
    if (meshRef.current) {
      const t = state.clock.getElapsedTime();
      meshRef.current.position.y = Math.sin(t * 1.6) * 0.2; // Float speed 1.6, amplitude 0.2
    }
  });

  return (
    <mesh ref={meshRef} geometry={geometry}>
      <meshStandardMaterial
        color="#0f2027"
        transparent
        opacity={0.88}
        emissive="#5d8cff"
        emissiveIntensity={2.0} // Adjusted for R3F scale
        roughness={0.02}
        metalness={0.0}
        side={THREE.DoubleSide}
      />
      <Eyes />
    </mesh>
  );
};

const Eyes = () => {
  const [blink, setBlink] = useState(false);

  useFrame((state) => {
    // Random blinking logic
    if (Math.random() > 0.995) {
      setBlink(true);
      setTimeout(() => setBlink(false), 150);
    }
  });

  const eyeScale = blink ? 0.1 : 1;

  return (
    <group>
      {/* Eye Sockets */}
      <mesh position={[-0.7, 0.6, 1.8]} scale={[1.1, 1.0, 0.6]}>
        <sphereGeometry args={[0.45, 32, 32]} />
        <meshBasicMaterial color="#000000" />
      </mesh>
      <mesh position={[0.7, 0.6, 1.8]} scale={[1.1, 1.0, 0.6]}>
        <sphereGeometry args={[0.45, 32, 32]} />
        <meshBasicMaterial color="#000000" />
      </mesh>

      {/* Glowing Eyes */}
      <group scale={[1, eyeScale, 1]}>
        <mesh position={[-0.7, 0.6, 1.95]}>
          <sphereGeometry args={[0.3, 32, 32]} />
          <meshBasicMaterial color="#00ff80" />{' '}
          {/* Greenish hue from reference 'eyeGlowColor' */}
        </mesh>
        <mesh position={[0.7, 0.6, 1.95]}>
          <sphereGeometry args={[0.3, 32, 32]} />
          <meshBasicMaterial color="#00ff80" />
        </mesh>
      </group>
    </group>
  );
};

const GhostScene = () => {
  const isMobile = useIsMobile();
  const particleCount = isMobile ? 40 : 150;

  return (
    <>
      <color attach="background" args={['#06071f']} />

      <ambientLight intensity={0.2} color="#0a0a2e" />
      <directionalLight position={[-8, 6, -4]} intensity={2} color="#4a90e2" />
      <directionalLight
        position={[8, -4, -6]}
        intensity={1.5}
        color="#50e3c2"
      />

      <group position={[0, 0, 0]}>
        <GhostMesh />
      </group>

      <Sparkles
        count={particleCount}
        scale={[15, 15, 15]}
        size={4}
        speed={0.4}
        opacity={0.5}
        color="#5d8cff"
      />

      <EffectComposer enableNormalPass={false}>
        <Bloom
          luminanceThreshold={0.5}
          mipmapBlur
          intensity={1.5}
          radius={0.6}
        />
        <Noise opacity={0.15} />
        <Vignette eskil={false} offset={0.1} darkness={1.1} />
        <Scanline density={1.5} opacity={0.3} />
      </EffectComposer>
    </>
  );
};

export default GhostScene;
