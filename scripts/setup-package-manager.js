...
function detectPackageManager() {
    ...
    if (process.env.CLAUDE_PACKAGE_MANAGER) {
        return { source: 'environment variable', value: process.env.CLAUDE_PACKAGE_MANAGER };
    }
    ...
    if (fs.existsSync(PROJECT_CONFIG_PATH)) {
        ...
        if (config.packageManager) {
            return { source: 'project config (.claude/package-manager.json)', value: config.packageManager };
        }
    }
    ...
    if (fs.existsSync(PACKAGE_JSON_PATH)) {
        ...
        if (pkg.packageManager) {
            return { source: 'package.json', value: pkg.packageManager.split('@')[0] };
        }
    }
    ...
    if (fs.existsSync(GLOBAL_CONFIG_PATH)) {
        ...
        if (config.packageManager) {
            return { source: 'global config (~/.claude/package-manager.json)', value: config.packageManager };
        }
    }
...
if (args.includes('--detect')) {
    ...
    console.log(`Detected package manager: ${result.value} (from ${result.source})`);
    ...
    fs.writeFileSync(GLOBAL_CONFIG_PATH, JSON.stringify({ packageManager: manager }, null, 2));
    ...
    if (!fs.existsSync(configDir)) fs.mkdirSync(configDir, { recursive: true });
    fs.writeFileSync(PROJECT_CONFIG_PATH, JSON.stringify({ packageManager: manager }, null, 2));
    console.log(`Project package manager set to: ${manager}`);
} else {
    console.log('Usage: node scripts/setup-package-manager.js [--detect | --list | --global <manager> | --project <manager>]');
}
