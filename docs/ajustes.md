...
- **App Router**:
  - Seções em **componentes dedicados**:
    - `SiteHeader.tsx`
    - `Hero.tsx` (+ `OrbCanvas.tsx` / `Hero3D.tsx`)
    - `HeroVideoThumb.tsx`
    ...
    - `SiteFooter.tsx`
- **3D**:
  - Cadeia clara: `Hero` → `OrbCanvas` → `TorusDan` / `GlassOrb` → GLB em `public/media//media/Torus_dan.glb`.
...
6.3 Cadeia Hero → OrbCanvas → GLB (gltfjsx)
