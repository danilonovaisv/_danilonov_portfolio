...
import HeroGlassCanvas from '@/components/three/HeroGlassCanvas'; // Renomeado para refletir o novo componente
...
const Hero: React.FC = () => {
  ...
  const videoStyle = useMemo(
    ...
    [videoScale, videoX, videoY, videoRadius]
  );

  // Variantes de Entrada (Initial Load)
  const flipContainer = useMemo(
    () => ({
      hidden: { opacity: 0 },
      visible: {
        opacity: 1,
  ...
  const flipItem: Variants = useMemo(
    () => ({
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 h-full items-center gap-12 md:gap-0">
            ...
            <div className="flex flex-col justify-center items-start relative z-20 order-2 md:order-1 pointer-events-auto">
              ...
              >
                ...
                <motion.span variants={flipItem} className="block">
                  não é só
                </motion.span>
                <motion.span variants={flipItem} className="block">
                  estética.
